...
L'algoritmo di \textit{stereo matching} che andiamo a presentare ha come input due immagini aeree di un'area geografica ed ha come obbiettivo il disegnare le linee di livello dell'area. Le due immagini devono essere uno \textit{stereo pair} e devono essere già calibrate e rettificate. L'algoritmo assume inoltre che le immagini siano in bianco e nero. % TODO: problem is called 'contour mapping' and is about extracting elevation information from brightness
% TODO: In questa fase, le immagini rappresentano la luminosità che c'è in ogni punto
...
La seguente funzione in *Lisp mostra l'implementazione originale dell'operazione. Questa funzione si occupa solo di un pixel. % se la diff di luminosita è maggiore della soglia...
...
Questa specifica funzione guarda solo a destra e a sinistra del pixel: questa e altre funzioni simili, che confronteranno anche gli altri lati (sopra, sotto e nelle diagonali), vengono combinate con la seguente funzione, per garantire il ritrovo di ogni contorno:
...
\section{Trovare corrispondenze tra i contorni}
